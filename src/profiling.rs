cfg_if::cfg_if! {
    if #[cfg(feature = "profiling")]
    {
        ...
        use tracy_client_sys::{
            ...
            ___tracy_gpu_time_data,
        };

        use gl::{GetInteger64v, TIMESTAMP, GenQueries, GetQueryObjectiv, GetQueryObjectui64v, QueryCounter, QUERY_RESULT_AVAILABLE, QUERY_RESULT};
        ...
        impl _Zone {
            pub fn new(loc_data: &___tracy_source_location_data, gpu: bool) -> Self {
                let context = unsafe {
                    ___tracy_emit_zone_begin(loc_data, 1)
                };
                if gpu {
        ...
        macro_rules! location_data {
            ($name: expr, $color: expr) => {{
                static LOC: crate::profiling::_LocationData =
                    crate::profiling::_create_location_data(
        ...
        macro_rules! tracy_zone {
            ($name: expr, $color: expr) => {
                let _tracy_zone = crate::profiling::_Zone::new(crate::profiling::location_data!($name, $color), false);
            };
            ($name: expr) => {
        ...
        macro_rules! tracy_gpu_zone {
            ($name: expr, $color: expr) => {
                let _tracy_zone = crate::profiling::_Zone::new(crate::profiling::location_data!($name, $color), true);
            };
            ($name: expr) => {
